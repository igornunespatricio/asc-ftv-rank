...
- Shared code used by more than one Lambda lives in a Lambda Layer at `backend/layers/shared/nodejs/node_modules/shared/` (Node.js layer convention — Lambda mounts layers at `/opt`, and `/opt/nodejs/node_modules` is on `NODE_PATH`, so consuming Lambdas import it as `require("shared")`, same as any npm package). Currently contains `normalizePath.js` (strips the API Gateway stage prefix from `event.requestContext.http.path` for named stages). Code used by only one Lambda (e.g. `backend/matches/calculate.js`) stays colocated with that Lambda instead — only promote something to the shared layer once a second Lambda actually needs it.
